...
export interface CameraProps extends ViewProps {
  ...
  /** Enable real-time face detection. iOS only (Apple Vision). Android fires `unavailable` via `onFaceDetectionInstallStatus`. */
  faceDetectionEnabled?: boolean;
  /** Throttle how often `onFaceDetected` emits. Defaults to 100 (~10 events/sec). iOS only. */
  faceDetectionThrottleMs?: number;
  /** Fires per frame while face detection is active on iOS; bounds are normalized 0–1 in preview space */
  onFaceDetected?: (event: OnFaceDetectedData) => void;
  /** **Android only.** Fires `'unavailable'` when face detection is enabled (not supported in this fork). */
